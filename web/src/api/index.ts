...
export const movementsApi = {
  list: (query: Query = {}) => apiRequest<Page<StockMovement>>('/movements', { query }), receive: (body: FreeReceiveRequest, idempotencyKey?: string) => apiRequest<FreeReceiveResponse>('/movements/receive', { method: 'POST', body, idempotencyKey }), issue: (body: IssueRequest, idempotencyKey?: string) => apiRequest<StockMovement[]>('/movements/issue', { method: 'POST', body, idempotencyKey }), transfer: (body: TransferRequest, idempotencyKey?: string) => apiRequest<StockMovement[]>('/movements/transfer', { method: 'POST', body, idempotencyKey }), returnItems: (body: ReturnRequest, idempotencyKey?: string) => apiRequest<StockMovement[]>('/movements/return', { method: 'POST', body, idempotencyKey }), rmaOut: (body: RmaMoveRequest, idempotencyKey?: string) => apiRequest<StockMovement[]>('/movements/rma-out', { method: 'POST', body, idempotencyKey }), rmaIn: (body: RmaMoveRequest, idempotencyKey?: string) => apiRequest<StockMovement[]>('/movements/rma-in', { method: 'POST', body, idempotencyKey }), adjust: (body: AdjustRequest, idempotencyKey?: string) => apiRequest<StockMovement>('/movements/adjust', { method: 'POST', body, idempotencyKey }), scrap: (body: ScrapRequest, idempotencyKey?: string) => apiRequest<StockMovement>('/movements/scrap', { method: 'POST', body, idempotencyKey }), reverse: (id: string, reason: string, idempotencyKey?: string) => apiRequest<StockMovement>(`/movements/${id}/reverse`, { method: 'POST', body: { reason }, idempotencyKey }), export: (format: 'csv', dateFrom?: string, dateTo?: string) => apiDownload('/movements/export', { format, date_from: dateFrom, date_to: dateTo }),
};
...
export const adminApi = { audit: (query: Query = {}) => apiRequest<Page<AuditEntry>>('/audit', { query }) };
